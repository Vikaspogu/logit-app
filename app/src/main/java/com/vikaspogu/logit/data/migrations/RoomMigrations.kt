...
val MIGRATION_5_6= object : Migration(5,6) {
    override fun migrate(db: SupportSQLiteDatabase) {
        ...
        db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS index_regional_type_name ON regional_type(name)")
        db.execSQL(
            """
          CREATE TABLE IF NOT EXISTS entries_new (
            ...
            type_id        INTEGER,
            regional_id    INTEGER,
            FOREIGN KEY(type_id)        REFERENCES types(id)            ON DELETE SET NULL ON UPDATE NO ACTION,
            FOREIGN KEY(attending_id)   REFERENCES attending(id)        ON DELETE SET NULL ON UPDATE NO ACTION,
            FOREIGN KEY(regional_id)    REFERENCES regional_type(id)    ON DELETE SET NULL ON UPDATE NO ACTION
          );
        """.trimIndent()
        )
        ...
        db.execSQL(
            """
          INSERT INTO entries_new (
            id, quantity, notes, asa, clinical, cvc, age, gender,
            attending_id, entry_date, type_id, regional_id
          )
          SELECT
            id, quantity, notes, asa, clinical, cvc, age, gender,
            attending_id, entry_date, type_id, regional_id
        ...
        db.execSQL("CREATE INDEX IF NOT EXISTS index_entries_type_id       ON entries(type_id);")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_entries_attending_id  ON entries(attending_id);")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_entries_regional_id   ON entries(regional_id);")
    }
}